<?php
/*
Plugin Name: MahiMahi Taxonomy Meta
based on http://www.deluxeblogtips.com/taxonomy-meta-script-for-wordpress by Rilwis
*/
...
class RW_Taxonomy_Meta {
	...
	function show( $tag, $taxonomy ) {
		...
		foreach ( $this->_fields as $field ) {
			...
			$meta = get_term_meta($_GET['tag_ID'], $field['id'], true); // get meta value for current field
	...
	function show_field_image( $field, $meta ) {
		...
		foreach ( $meta as $att ) {
			$src = get_thumbnail(null, null, array('attachment_id' => $att, 'only_src' => true, 'width' => 200));
	...
	function save_field( &$meta, $field, $old, $new ) {
		...
		if( in_array($_POST['taxonomy'], $this->_taxonomies) ):
			delete_term_meta($_POST['tag_ID'], $field['id']);
			add_term_meta($_POST['tag_ID'], $field['id'], $new);
		endif;
	}
