...
function parse_time($str, $format = "%d/%m/%Y") {
	
	$parsed = strptime($str, $format);
	
	$time = mktime($parsed['tm_hour'], $parsed['tm_min'], $parsed['tm_sec'], 1+$parsed['tm_mon'], $parsed['tm_mday'], 1900 + $parsed['tm_year']);
	
	return $time;
}

if ( !function_exists('get_firstmonday')):
