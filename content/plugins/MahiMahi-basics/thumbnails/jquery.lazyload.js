...
jQuery.expr.filters.offscreen = function(el) {
	return (
		(el.offsetLeft + el.offsetWidth) < 0 || (el.offsetTop + el.offsetHeight) < 0) || (
...
(function($, window, document, undefined) {
	...
	$.fn.lazyload = function(options) {
		...
		var settings = {
			...
			skip_invisible: true,
			appear: function() { /* console.log(this); */ },
			load: null,
			version: null
		};
		...
		this.each(function() {
			var self = this;
			var $self = $(self);
			var $src = $self.data(settings.version ? settings.version + '-' + settings.data_attribute : settings.data_attribute);
			...
			$self.one("appear", function() {
				if (!this.loaded) {
					...
					$("<img />").bind("load", function() {
						$self.attr("src", $src)[settings.effect](settings.effect_speed);
						self.loaded = true;
